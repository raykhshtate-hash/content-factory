...
ALLOWED_FONTS = {"Montserrat", "Rubik", "PT Sans", "Open Sans", "Roboto", "Roboto Condensed", "Lato", "Raleway", "Lora"}
...
=== ПРИОРИТЕТ БРИФА (ЧИТАЙ ПЕРВЫМ) ===

Сценарий (scenario_text) — это НЕ просто тема, это директивы режиссёра.
Если бриф ЯВНО указывает что-то про переходы, стикеры или ритм — следуй
брифу, не своим mood-эвристикам. Бриф ВЫИГРЫВАЕТ против любых defaults.

Распознавай и выполняй буквально:

ПЕРЕХОДЫ — если в брифе встречается:
- "резы", "чёткие резы", "hard cuts", "без переходов", \
"без долгих переходов", "минимум переходов" → \
overall_style = "clean", все transition: null
- "плавные", "мягкие переходы", "soft" → overall_style = "soft"
- "разнообразные переходы", "динамично", "energetic" → \
overall_style = "dynamic"
- "на стыке двух видео мягкий переход", "soft на video boundary" → \
определи где меняется источник (по описанию клипов / video_index если \
видно) и поставь fade ТОЛЬКО там, остальные null

СТИКЕРЫ — если в брифе встречается:
- "(\\d+) стикер", "стикеры на (\\d+) названиях", "(\\d+) штуки стикеров" → \
ровно столько overlay объектов, не больше и не меньше
- "без стикеров", "no stickers", "0 стикеров" → overlays: []
- "стикеры точечно", "не перегружать" → 1-2 стикера максимум
- "стикеры на названиях процедур / продуктов / темах" → стикер на каждом \
упоминании этого объекта в сценарии

РИТМ — если в брифе встречается:
- "быстрый темп", "нарастающий темп" → короткие клипы, dynamic анимация
- "спокойный", "медленный" → длинные клипы, fade

Если в брифе НЕТ явных директив на эту область — действуй по mood-defaults
ниже. Бриф НЕ обязан указывать всё; молчание = действуй по дефолту.
...
- Lora — серифный, элегантный курсив, тёплый и premium (ПРЕДПОЧИТАЙ для красивого/женственного контента)
- Montserrat — чистый, современный, жирный (default для нейтрального контента)
- Rubik — современный, чуть округлый, дружелюбный
- PT Sans — нативный русский, чёткий, профессиональный
- Roboto — стандарт для видео, идеальная читаемость
- Roboto Condensed — компактный, для длинных фраз
- Lato — элегантный, современный, мягкий
- Open Sans — нейтральный, читаемый
- Raleway — тонкий, элегантный

Верни выбранный шрифт в поле font_family.

=== ЦВЕТ СУБТИТРОВ ===

Базовый текст всегда белый полупрозрачный. Ты выбираешь цвет АКЦЕНТА — \
цвет активного слова при караоке. Это создаёт эффект: белый текст, \
а текущее слово подсвечивается акцентом.

Палитра акцентов (premium):
- #FFFFFF — белый (чистый, минималистичный — активное слово ярче остальных)
- #F5F0EB — тёплый крем (уют, мягкость, женственность)
- #D4AF37 — приглушённое золото (luxury, для premium контента)
- #E0B0FF — лавандовый (нежный, soft)
- #B8D4E3 — пыльный голубой (свежий, спокойный)
- #FFE600 — жёлтый (ТОЛЬКО юмор)
- #FF3232 — красный (ТОЛЬКО драма)
